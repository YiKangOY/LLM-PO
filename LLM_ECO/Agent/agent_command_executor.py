...
def _formulate_tcl_command(tcl_command, iteration):
    ...
    if "timing" in tcl_command:
        tcl_command += " -verbose -unfixable_reasons_format text -unfixable_reasons_prefix {}/reports/unfix_timing_{}".format(workspace, iteration)

    if "buffer_insertion" in tcl_command:
        tcl_command += f" -buffer_list {BUFFER_LIST} "
    if "opt_power" in tcl_command:
        tcl_command += " -verbose > {}/reports/fix_power_{}.txt;".format(workspace, iteration)
    if "opt_area" in tcl_command:
        tcl_command += " -verbose > {}/reports/fix_area_{}.txt;".format(workspace, iteration)
    elif "opt_timing" in tcl_command:
        tcl_command += " > {}/reports/fix_timing_{}.txt;".format(workspace, iteration)
...
def execute_tcl_command(tcl_command, command_type, iteration, round_index, run_paths=None, pt_server=None):
    ...
    command = f'cd {run_paths.workspace} && pt_shell -f {script_path} | tee {log_path}'
...
def _parse_execution_time(command_type, iteration):
    ...
    if 'timing' in command_type.lower():
        report_file = os.path.join(workspace, f'reports/fix_timing_{iteration}.txt')
    elif 'power' in command_type.lower():
        report_file = os.path.join(workspace, f'reports/fix_power_{iteration}.txt')
    elif 'area' in command_type.lower():
        report_file = os.path.join(workspace, f'reports/fix_area_{iteration}.txt')
