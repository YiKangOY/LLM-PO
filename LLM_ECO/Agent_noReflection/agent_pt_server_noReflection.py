...
from config_noReflection import PT_SERVER_CONFIG
...
class PTServerManager:
    ...
    def start(self):
        ...
        print("[PTSERVER] starting pt_shell with init script {}".format(self.init_tcl_path), flush=True)
        self.process = subprocess.Popen(
            ["bash", "-lc", f"pt_shell -f {self.init_tcl_path} | tee pt.log"],
            cwd=self.run_paths.workspace,
        )
        if self._wait_for_port():
            print("[PTSERVER] port open on {}:{}".format(
                self.run_paths.server_host,
                self.run_paths.server_port
            ), flush=True)
            return True
        print("[PTSERVER] failed to open port within {}s".format(self.start_timeout), flush=True)
    ...
    def run_script(self, script_path):
        ...
        print("[PTSERVER] dispatching script {}".format(script_path), flush=True)
        response = self._send_command(command)
        print("[PTSERVER] response for {}: {}".format(script_path, response), flush=True)
    ...
    def _send_command(self, command):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            print("[PTSERVER] connecting to {}:{} with timeout {}s".format(
                self.run_paths.server_host,
                self.run_paths.server_port,
            ...
            if not command.endswith("\n"):
                command = command + "\n"
            print("[PTSERVER] sending command: {}".format(command.strip()), flush=True)
