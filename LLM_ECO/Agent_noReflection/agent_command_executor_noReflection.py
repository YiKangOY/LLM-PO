...
from config_noReflection import BUFFER_LIST, base_path
from report_parsers import parse_elapsed_time
from agent_run_paths_noReflection import AgentRunPaths
from agent_pt_server_noReflection import PTServerManager
...
def execute_tcl_command(tcl_command, command_type, iteration, round_index, run_paths=None, pt_server=None):
    ...
    print("[EXECUTOR][round {}][iter {}] command_type={}".format(
        round_index,
        iteration,
        command_type
    ), flush=True)
    print("[EXECUTOR][round {}][iter {}] original command: {}".format(
        round_index,
        iteration,
        ori_tcl_command
    ), flush=True)
    print("[EXECUTOR][round {}][iter {}] formatted command: {}".format(
        round_index,
        iteration,
        formatted_command
    ), flush=True)
    ...
    print("[EXECUTOR][round {}][iter {}] wrote script {}".format(
        round_index,
        iteration,
        script_path
    ), flush=True)
    ...
    if server_ready:
        print("[EXECUTOR][round {}][iter {}] using persistent PT server".format(
            round_index,
            iteration
        ), flush=True)
        ...
        if success:
            ...
            print("[EXECUTOR][round {}][iter {}] PT server execution succeeded in {:.2f}s".format(
                round_index,
                iteration,
                end_time - start_time
            ), flush=True)
        ...
        print("[EXECUTOR][round {}][iter {}] PT server execution failed, falling back to standalone pt_shell. Response: {}".format(
            round_index,
            iteration,
            server_response
        ), flush=True)
    ...
    print("[EXECUTOR][round {}][iter {}] running fallback command: {}".format(
        round_index,
        iteration,
        command
    ), flush=True)
    ...
    if exit_code == 0:
        execution_time = _parse_execution_time(command_type, iteration)
        print("[EXECUTOR][round {}][iter {}] parsed execution_time={} from reports".format(
            round_index,
            iteration,
            execution_time
        ), flush=True)
    ...
    print("[EXECUTOR][round {}][iter {}] fallback execution success={} exit_code={}".format(
        round_index,
        iteration,
        success,
        exit_code
