...
class BOOMAgent(Agent):
    def __init__(self, configs, env):
        super(BOOMAgent, self).__init__(configs)
        self.device = torch.device(
            "cuda" if self.configs["algo"]["use-cuda"] else "cpu"
        )
        self.envs = make_a3c_vec_envs(self.configs, env)
        observation_dim = space_dim(self.envs.observation_space)
        action_dim = space_dim(self.envs.action_space)
    ...
    def optimize_ppo(self, preference, reward, adv):
        """
            Update that reuses scalarized rewards via
            preference weighting. Gradients are clipped for stability.
        """
        buffer = self.buffer.batch_pool
        state = array_to_tensor(buffer["state"])
        action = array_to_tensor(buffer["action"], fmt=int)
        old_log_prob = array_to_tensor(buffer["log-prob"])
        preference = array_to_tensor(preference)
        reward = array_to_tensor(reward)
        adv = array_to_tensor(adv)
        ...
        policy_logits = self.adjust_action_space(policy_logits)
        policy = F.softmax(
            policy_logits / (self.temperature if self.training else 1),
            dim=-1
        )
        ...
        self.critic_loss = torch.tensor(0.0)
        ...
        for _ in range(self.ppo_epoch):
            ...
            for start in range(0, batch_size, self.ppo_batch_size):
                ...
                value_w = torch.bmm(
                    value[idx].unsqueeze(1),
                    preference[idx].unsqueeze(2)
                ).squeeze()
                ...
                loss = actor_loss + \
                    self.ppo_value_coef * critic_loss - \
                    self.alpha * entropy
