...
class Agent(ABC):
    ...
    @property
    def algo_name(self):
        return self.configs["algo"].get("algo", "a3c").lower()

    @property
    def is_ppo(self):
        return self.algo_name == "ppo"
    ...
    @property
    def ppo_clip(self):
        return self.configs["algo"]["train"].get("ppo-clip", 0.2)

    @property
    def ppo_epoch(self):
        return self.configs["algo"]["train"].get("ppo-epoch", 1)

    @property
    def ppo_batch_size(self):
        return self.configs["algo"]["train"].get("ppo-batch-size", self.num_parallel * self.num_step * self.sample_size)

    @property
    def ppo_value_coef(self):
        return self.configs["algo"]["train"].get("ppo-value-coef", 0.5)
