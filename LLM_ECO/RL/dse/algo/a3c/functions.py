...
def space_dim(space) -> int:
    """
    Convert a Gymnasium/Gym space or numeric value into a flat dimension.
    Handles Discrete, MultiDiscrete, MultiBinary, Tuple/Dict, and Box spaces.
    """
    if hasattr(space, "n"):  # Discrete
        return int(space.n)
    if hasattr(space, "shape") and space.shape is not None:
        return int(np.prod(space.shape))
    if hasattr(space, "spaces"):  # Tuple/Dict collection
        return int(sum(space_dim(s) for s in getattr(space, "spaces")))
    # Fallback: assume numeric dimension already provided
    return int(space)
