...
class MOGP(ExactGP, MultiTaskGPyTorchModel):
    ...
    def __init__(
        ...
        # required by botorch MultiTaskGPyTorchModel interface for posterior checks
        self.num_non_task_features = d
        ...
        # Identity mapping for task indices (required by MultiTaskGPyTorchModel API)
        self._task_idcs = torch.arange(self._num_outputs, dtype=torch.long)
    ...
    def _map_tasks(self, task_values: torch.Tensor) -> torch.Tensor:
        """
        Botorch expects a mapping from requested task values to model task ids.
        This model keeps tasks in-order, so the mapping is identity.
        """
        return task_values
    ...
    @classmethod
    def construct_inputs(cls, training_data: Any, **kwargs) -> Dict[str, Any]:
        """
        Construct kwargs for the `Model` from training data and other options.

        Args:
            training_data: Container with data for single outcome
...
class DKLGP(object):
    def __init__(self, x: torch.Tensor, y: torch.Tensor, **kwargs: Dict):
        ...
        self.feature_shift: Optional[torch.Tensor] = None
        self.feature_scale: Optional[torch.Tensor] = None
        x = self.forward_mlp(x, update_stats=True)
    ...
    def forward_mlp(self, x: torch.Tensor, update_stats: bool = False) -> torch.Tensor:
        x = x.to(self.device)
        x = self.mlp(x)
        if update_stats or self.feature_shift is None or self.feature_scale is None:
            with torch.no_grad():
                shift = x.detach().min(0)[0]
                scale = (x.detach().max(0)[0] - shift).clamp_min(1e-6)
            self.feature_shift = shift
            self.feature_scale = scale
        shift = self.feature_shift.to(device=x.device, dtype=x.dtype)
        scale = self.feature_scale.to(device=x.device, dtype=x.dtype).clamp_min(1e-6)
        return 2 * ((x - shift) / scale) - 1
    ...
    def predict(self, x: torch.Tensor) -> torch.Tensor:
        def _transform_ylayout(y: torch.Tensor) -> torch.Tensor:
            ...
            return torch.cat([y[i].unsqueeze(1) for i in range(self.n_target)], dim=1)
    ...
    def sample(self, x: torch.Tensor, y: torch.Tensor) \
        ...
        _x = self.forward_mlp(x.to(self.device).float())
