...
def parse_args() -> argparse.Namespace:
    def initialize_parser(
        ...
        parser.add_argument(
            ...
            type=int,
            default=None,
            help="Override BO max steps (episodes)"
        )
        parser.add_argument(
            "--design",
            type=str,
            default=None,
            help="Design name to select from configs/design_configs.py"
        )
        parser.add_argument(
            "--num-envs",
            type=int,
            default=None,
            help="Override parallel envs for BO evaluations"
        )
        parser.add_argument(
            "--use-cuda",
            action="store_true",
            help="Enable CUDA for DKL-GP if available"
        )
        parser.add_argument(
            "--gpu-id",
            type=int,
